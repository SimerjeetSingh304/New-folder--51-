...
import { Lightbulb, CheckCircle2, Hourglass, TrendingUp, ChevronDown, Search, SearchX } from 'lucide-react';
import PageWrapper from '../components/layout/PageWrapper';
import Card from '../components/ui/Card';
import { APTITUDE_DATA, APTITUDE_CATEGORIES, APTITUDE_CATEGORY_COLORS } from '../utils/aptitudeData';

const STORAGE_KEY = 'devtracker_aptitude_completed';
...
const Aptitude = () => {
  ...
  const totalTopics = APTITUDE_DATA.length;
  const completedCount = completed.size;
  const remainingCount = totalTopics - completedCount;
  const overallPct = Math.round((completedCount / totalTopics) * 100);

  const catStats = useMemo(() => {
    return APTITUDE_CATEGORIES.filter((c) => c !== 'All').map((cat) => {
      const items = APTITUDE_DATA.filter((s) => s.category === cat);
      const done = items.filter((s) => completed.has(s.id)).length;
      return { cat, total: items.length, done, pct: items.length > 0 ? Math.round((done / items.length) * 100) : 0 };
    });
  }, [completed]);

  const catStyle = (cat) => {
    return APTITUDE_CATEGORY_COLORS[cat] || APTITUDE_CATEGORY_COLORS['Quantitative'];
  };
      ...
      <motion.div initial="hidden" animate="show" variants={containerVariants}>
        ...
        <motion.div variants={itemVariants} style={{ marginBottom: '32px' }}>
          <h1 style={{ margin: 0, fontSize: '32px', fontWeight: '800', color: 'var(--color-base-100)', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Lightbulb size={28} style={{ color: 'var(--color-accent-500)' }} />
            Aptitude Tracker
          </h1>
          <p style={{ margin: '6px 0 0', color: 'var(--color-base-400)', fontSize: '15px' }}>
            Comprehensive syllabus for placement tests. Track your learning progress and stay consistent.
          </p>
        </motion.div>
        ...
        <motion.div variants={itemVariants} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          {[
            { label: 'Total Topics', value: totalTopics, color: '168, 85, 247', icon: <Lightbulb size={24} /> },
            { label: 'Mastered', value: completedCount, color: '16, 185, 129', icon: <CheckCircle2 size={24} /> },
            ...
            {filtered.length === 0 ? (
              <motion.div key="empty" variants={itemVariants} exit="exit" layout>
                <Card hover={false} style={{ textAlign: 'center', padding: '64px 24px', borderStyle: 'dashed' }}>
                  ...
                  <h3 style={{ margin: '0 0 8px', color: 'var(--color-base-100)', fontSize: '20px' }}>No topics match your filter</h3>
                          ...
                          {isExpanded && (
                            ...
                            >
                              <div style={{
                                ...
                                {item.subtopics.split(' · ').map((sub, i) => (
                                  <span key={i} style={{
        ...
        {filtered.length > 0 && (
          <motion.p variants={itemVariants} style={{ textAlign: 'center', color: 'var(--color-base-500)', fontSize: '13px', marginTop: '32px', fontWeight: 500 }}>
            Showing {filtered.length} of {totalTopics} topics
          </motion.p>
        )}
      </motion.div>
    </PageWrapper>
